...
2. Открыть KaiTOR Diplomacy и оставить экран открытым не менее 60 секунд. Затем закрыть/открыть его 10 раз.
3. На главном экране: выбрать державу, сменить длительность пакта, проверить предложение NAP и разрыв NAP (при допустимых игровых условиях).
4. Открыть «Семейные дела»: пройти выбор члена рода -> другого дома -> кандидата; отмена/возврат не должны зависать или закрывать игру.
5. В принадлежащем игроку городе/замке открыть смену национальности/культуры поселения и проверить возврат в меню.
6. Сохранить игру, перезагрузить сохранение и повторно открыть UI.
7. При появлении нового AI Realm House проверить KaiTORRealmHouse.log: после NATIVE_FACTORY_OK ожидается PROMOTE_OCCUPATION_OK (если требовалось), затем REALM_CREATE_SUCCESS; лидер нового клана должен быть Lord.
8. При установленном KaiTOR Co-op открыть/закрыть оба интерфейса по очереди; movie/layer Diplomacy не должны пересекаться с Co-op.
