...
2. Открыть Королевство -> Дипломатия, выбрать другую державу и нажать «Открыть панель KaiTOR». Оставить экран открытым не менее 60 секунд, затем закрыть/открыть его 10 раз.
3. На панели KaiTOR: выбрать державу, сменить длительность пакта, проверить предложение NAP и разрыв NAP при допустимых игровых условиях. Каждое нажатие должно либо создать council decision, либо показать явную причину отказа.
4. В таверне проверить «Принять в род»: открыть список кандидатов, отменить, вернуться назад, затем выполнить одно допустимое усыновление.
...
- native Kingdom Diplomacy -> панель KaiTOR;
- NAP create/review/break -> council decision или явное сообщение;
- tavern -> adoption -> candidate/apply/back;
...
- [ ] В таверне есть единственный settlement-family пункт «Принять в род».
- [ ] «Принять в род» открывает список кандидатов или явное сообщение, если кандидатов нет.
- [ ] «Назад» из adoption-menu возвращает именно в таверну.
...
- [ ] Королевство -> Дипломатия -> выбранная держава показывает «Открыть панель KaiTOR».
- [ ] Кнопка открывает KaiTORDiplomacyHubUIMovie; выбранная держава уже подставлена в панель.
- [ ] «Выбрать державу» открывает список и обновляет выбранную державу.
- [ ] «Срок пакта» меняется 30/60/90/180.
- [ ] «Предложить пакт» создаёт council decision либо показывает явную причину отказа.
- [ ] «Разорвать пакт» создаёт council decision либо показывает явную причину отказа.
- [ ] «Сменить культуру поселения» либо открывает подтверждение, либо показывает явную причину недоступности.
- [ ] «Обновить» меняет статус и показывает подтверждение обновления.
- [ ] «Закрыть» снимает focus/input restrictions; вращение карты после закрытия работает.
- [ ] На панели НЕТ старых кнопок «Открыть семейные дела» и «Открыть классическое меню KaiTOR».
- [ ] kaitor_diplomacy.ui_status показывает актуальные behaviors и точки входа.
- [ ] При невозможности открыть native council UI нет бесшумного клика: появляется fallback-сообщение.

## Gauntlet UI / anti-collision live-test
- [ ] Панель открывается из native Kingdom Diplomacy, а не из города/town_outside/замка.
- [ ] В городе, town_outside и замке нет старой кнопки «KaiTOR: Дипломатия и династия».
- [ ] Кнопка native Kingdom Diplomacy открывает именно KaiTORDiplomacyHubUIMovie, а не Co-op movie.
- [ ] Co-op main menu / Join / Options продолжают открывать CoopConnectionUIMovie и CoopOptionsUIMovie без изменений.
- [ ] Статический safe-layout не содержит ScrollablePanel/NavigatableListPanel/ItemTemplate/динамических IsVisible/IsDisabled bindings.
- [ ] «Выбрать державу» обновляет NAP/trust/breach/cooldown.
...
- [ ] Семейная сводка на панели информационная; adoption выполняется только из таверны.
- [ ] «Сменить культуру поселения» закрывает screen только после проверки наличия behavior и затем даёт dialog/result.
- [ ] После закрытия панели мышь и карта не остаются заблокированы.
- [ ] После save/load NAP, династические связи и UI runtime продолжают работать.
